...
if __name__ == "__main__":
    ...
    os.makedirs(f"../../experiment/HEVC/{input_format}/",exist_ok=True) 
    enc_dir = f"../../experiment/HEVC/{input_format}/enc/"  
    ...
    if input_format=='RGB444':
        for qp in qplist:
            for seq in seqlist:
                ...
                subprocess.call("./hevc/encode_rgb444.sh "+qp+" "+str(frames)+" "+str(width)+" "+str(height)+" "+original_seq+" "+enc_dir+" "+test_dataset+'_'+seq+" "+' &') 
                print(seq+"_"+qp+" submiited")
        
    elif input_format=='YUV420':
        org_yuv_dir = f"../../experiment/HEVC/{input_format}/org_yuv/"
        ...
        for qp in tqdm(qplist):
            for seq in tqdm(seqlist):            
                oriyuv_path=org_yuv_dir+test_dataset+'_'+str(seq)+'_'+str(width)+'x'+str(height)+'_25_8bit_420.yuv'
                subprocess.call(["./hevc/encode_yuv420.sh",str(qp),str(frames),str(width),str(height),oriyuv_path, enc_dir, test_dataset+'_'+seq,'&'])   
                ########################       
