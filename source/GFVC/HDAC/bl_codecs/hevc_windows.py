...
class HEVC_Encoder:
    ...
    def run(self):
        # 使用FFmpeg替代HEVC_HM编码器，因为后者是Linux-only的ELF文件
        enc_start = time.time()
        
        # 使用FFmpeg进行HEVC编码
        cmd = ['ffmpeg', '-nostats', '-loglevel', 'error', 
               '-f', 'rawvideo', '-pix_fmt', 'yuv420p', 
               '-s:v', f'{self.frame_dim[0]}x{self.frame_dim[1]}', 
               '-r', str(self.fps), '-i', self.in_yuv_path,
               '-c:v', 'libx265', '-crf', str(self.qp), 
               '-f', 'hevc', self.ostream_path]
        
        subprocess.call(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        enc_time = time.time() - enc_start
        
        # 计算比特数
        bits = os.path.getsize(self.ostream_path) * 8 if os.path.exists(self.ostream_path) else 0
        
        # 使用FFmpeg进行解码
        dec_start = time.time()
        dec_cmd = ['ffmpeg', '-nostats', '-loglevel', 'error', 
                   '-i', self.ostream_path, 
                   '-f', 'rawvideo', '-pix_fmt', 'yuv420p', 
                   self.dec_yuv_path]
