...
class Logger:
    ...
    def log_scores(self, loss_names:List[str]):
        if not self.loss_list:
            print("Warning: No loss data to log", file=self.log_file)
            self.log_file.flush()
            return
        ...
        if len(loss_mean) == 0:
            print("Warning: Empty loss mean array", file=self.log_file)
            self.log_file.flush()
            return
        ...
        try:
            ...
            print(f"Error in log_scores: {str(e)}", file=self.log_file)

    def visualize_rec(self, inp:Dict[str, torch.Tensor], out:Dict[str, torch.Tensor], 
                      name:str=None):
        if inp is None or out is None:
            print("警告：输入为None，跳过可视化", file=self.log_file)
            self.log_file.flush()
            return None
    ...
    def log_epoch(self, epoch, models=None, inp=None, out=None, name=None):
        ...
        if models is not None:
            ...
            if (self.epoch + 1) % self.checkpoint_freq == 0:
                self.save_cpk()
        if self.names is not None and len(self.loss_list) > 0:
