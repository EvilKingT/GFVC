...
class ReferenceImageCoder:
    ...
    def vtm_yuv_compress(self, frame_idx_str:str)->tuple:
        # 使用FFmpeg替代VTM编码器，因为后者是Linux-only的ELF文件
        
        # 输入和输出文件路径
        input_yuv = self.img_output_dir + 'frame' + frame_idx_str + '_org.yuv'
        output_bin = self.img_output_dir + 'frame' + frame_idx_str + '.bin'
        rec_yuv = self.img_output_dir + 'frame' + frame_idx_str + '_rec.yuv'
        
        # 使用FFmpeg进行编码
        cmd_encode = ['ffmpeg', '-nostats', '-loglevel', 'error',
                      '-f', 'rawvideo', '-pix_fmt', 'yuv420p',
                      '-s:v', f'{self.width}x{self.height}', '-i', input_yuv,
                      '-c:v', 'libx265', '-crf', str(self.qp),
                      '-f', 'hevc', output_bin]
        subprocess.call(cmd_encode, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        
        # 计算比特数
        bits = os.path.getsize(output_bin) * 8 if os.path.exists(output_bin) else 0
        ...
        rec_ref_rgb = yuv420_to_rgb444(rec_yuv, self.width, self.height, 0, 1, False, False)
    ...
    def vtm_rgb_compress(self, frame_idx_str)->tuple:
        ...
        os.system("./image_codecs/vtm/encode_rgb444.sh "+self.img_output_dir+'frame'+frame_idx_str+" "+str(self.qp)+" "+str(self.width)+" "+str(self.height))   ########################
