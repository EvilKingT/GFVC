...
if __name__ == "__main__":
    ...
    parser.add_argument("--original_seq", default='./testing_sequence/001_256x256.rgb', type=str, 
                        help="path to the input testing sequence (支持格式: .rgb, .yuv, .mp4, .avi, .mkv)")
    ...
    parser.add_argument("--iframe_qp", default=32, help="the quantization parameters for encoding the Intra frame")
    ...
    parser.add_argument("--adaptive_thresh", default=0, type=float,help="Reference selection threshold")
    ...
    if original_seq.endswith('.yuv') or original_seq.endswith('.YUV'):
        ...
        for frame in rgb_frames:
            ...
            listG.append(G)
            listB.append(B)
    elif original_seq.endswith(('.mp4', '.avi', '.mkv')):
        # 对于MP4/AVI/MKV等压缩格式文件，使用OpenCV解码
        print(f"读取视频文件: {original_seq}")
        cap = cv2.VideoCapture(original_seq)
        if not cap.isOpened():
            raise IOError(f"无法打开视频文件: {original_seq}")
        ...
        # 确保读取了足够的帧
        if len(listR) < frames:
            raise ValueError(f"视频只有{len(listR)}帧，但需要{frames}帧")
        
        print(f"成功读取并处理了{len(listR)}帧视频")
    else:
