...
def read_preencoded_video(video_path: str, width: int, height: int, num_frames: int)->np.ndarray:
    """
    读取预编码的MP4视频并转换为所需的格式
    
    Args:
        video_path: 预编码视频的路径（MP4格式）
        width: 目标宽度
        height: 目标高度
        num_frames: 需要读取的帧数
        
    Returns:
        np.ndarray: 转换后的视频帧，形状为(N, 3, H, W)
    """
    # 检查文件是否存在
    if not os.path.exists(video_path):
        raise FileNotFoundError(f"预编码视频文件不存在: {video_path}")
    
    # 检查文件扩展名
    ext = os.path.splitext(video_path)[1].lower()
    if ext not in ['.mp4', '.avi', '.mkv']:
        raise ValueError(f"预编码视频格式不支持，请使用MP4/AVI/MKV格式: {video_path}")
    ...
    while frame_count < num_frames:
        ...
        try:
            ...
            raise RuntimeError(f"帧转换失败 (帧{frame_count}): {str(e)}")
    ...
    if len(frames) < num_frames:
        raise ValueError(f"预编码视频只有{len(frames)}帧，但需要{num_frames}帧")
    
    # 转换为所需格式: (N, 3, H, W)
    frames_array = np.array(frames)
    frames_transposed = np.transpose(frames_array, [0, 3, 1, 2])
    
    print(f"成功读取并处理了{len(frames)}帧预编码视频")
    return frames_transposed
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_layer_seq", default=None, type=str,help="Path to pre-encoded base layer video (MP4 format)")
    ...
    base_layer_seq = opt.base_layer_seq
    ###
    ...
    with torch.no_grad():
        ...
        if use_base_layer == 'ON':
            if base_layer_seq is not None:
                # 方案1: 使用预编码的基础层视频
                print(f'使用预编码的基础层视频: {base_layer_seq}')
                # 读取并处理预编码视频
                bl_frames = read_preencoded_video(base_layer_seq, width, height, frames)
                # 计算基础层比特数（使用文件大小作为近似）
                bl_bits = os.path.getsize(base_layer_seq) * 8
                sum_bits += bl_bits
